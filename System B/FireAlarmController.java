			...
			while ( !Done )
			{
				...
				for ( int i = 0; i < qlen; i++ )
				{
					...
					if ( Msg.GetMessageId() == 12 )
					{
						...
						{
							state = true;
							mw.WriteMessage("Received fire message" );
						...
						{
							state = false;
							mw.WriteMessage("Received no fire message" );
					...
					if(Msg.GetMessageId() == 44 && Msg.GetMessage().equalsIgnoreCase("ON")){
						state = false;
				...
				if (state)
				{
					...
					di.SetLampColorAndMessage("Fire Alarm On", 3);

				} else {

					// Set to black, sprinkler is off
					di.SetLampColorAndMessage("Fire Alarm Off", 1);
